...
describe('TenancyModule', () => {
  it('registers controllers, providers, and exported services', () => {
    expect(
      Reflect.getMetadata(MODULE_METADATA.CONTROLLERS, TenancyModule),
    ).toEqual([MembershipsController, OrganizationsControllers]);
    expect(Reflect.getMetadata(MODULE_METADATA.EXPORTS, TenancyModule)).toEqual(
      [OrganizationService, MembershipService, MembershipRepository],
    );
    expect(
      Reflect.getMetadata(MODULE_METADATA.PROVIDERS, TenancyModule),
    ).toEqual([
      MembershipService,
      OrganizationService,
